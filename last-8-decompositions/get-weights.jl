...
data_path = raw"C:\Users\ellio\git\gcn-dmpc\final-analysis\last-8-decompositions\ML-preds-GCN-train-floats-and-labels.csv"
...
df7 = filter(row -> row[label_col] == 7, df)
df8 = filter(row -> row[label_col] == 8, df)
...
CSV.write(raw"C:\Users\ellio\git\gcn-dmpc\final-analysis\last-8-decompositions\edge-weights-210.csv", DataFrame(weights=weights))