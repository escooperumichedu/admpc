...
function dmpc1_mpc(x_init, x_sp, u_sp, max_coordinations)
    ...
    function subsystem2()
        ...
        # Obtain optimal inputs and corresponding obtained state predictions
        u_opt = Vector(JuMP.value.(F_f2)), Vector(JuMP.value.(F_1)), Vector(JuMP.value.(Q_2))
    ...
    function subsystem3()
        ...
        @NLconstraints MPC begin
            ...
            # Discretized governing differential equations for temperature
            dT_3dt[k=0:N-1], T_3[k] + ((F_2[k] / V_3[k]) * (x.T_2[k+1] - T_3[k]) + Q_3[k] / (rho * cp * V_3[k])) * dt == T_3[k+1]

            # Discretized governing differential equations for concentration A

            dx_A3dt[k=0:N-1], x_A3[k] + ((F_2[k] / V_3[k]) * (x.x_A2[k+1] - x_A3[k]) - ((u.F_R[k+1] + u.F_R[k+1] * epsilon) / V_3[k]) * ((alpha_A * x_A3[k]) / (alpha_A * x_A3[k] + alpha_B * x_B3[k] + alpha_C * (1 - x_A3[k] - x_B3[k])) - x_A3[k])) * dt == x_A3[k+1]
            dx_B3dt[k=0:N-1], x_B3[k] + ((F_2[k] / V_3[k]) * (x.x_B2[k+1] - x_B3[k]) - ((u.F_R[k+1] + u.F_R[k+1] * epsilon) / V_3[k]) * ((alpha_B * x_B3[k]) / (alpha_A * x_A3[k] + alpha_B * x_B3[k] + alpha_C * (1 - x_A3[k] - x_B3[k])) - x_B3[k])) * dt == x_B3[k+1]
        ...
        # Obtain optimal inputs and corresponding obtained state predictions
        u_opt = Vector(JuMP.value.(F_2)), Vector(JuMP.value.(F_3)), Vector(JuMP.value.(Q_3))
