...
function dmpc3_mpc(x_init, x_sp, u_sp, max_coordinations)
    ...
    function subsystem1()
        ...
        @NLconstraints MPC begin
            ...
            dx_A1dt[k=0:N-1], x_A1[k] + ((F_f1[k] / x.V_1[k+1]) * (x_Af - x_A1[k]) + (F_R[k] / x.V_1[k+1]) * ((alpha_A * x_A3[k]) / (alpha_A * x_A3[k] + alpha_B * x_B3[k] + alpha_C * (1 - x_A3[k] - x_B3[k])) - x_A1[k]) - k_1 * exp(-E_1 / (R * x.T_1[k+1])) * x_A1[k]) * dt == x_A1[k+1]
            dx_B1dt[k=0:N-1], x_B1[k] + ((F_f1[k] / x.V_1[k+1]) * (x_Bf - x_B1[k]) + (F_R[k] / x.V_1[k+1]) * ((alpha_B * x_B3[k]) / (alpha_A * x_A3[k] + alpha_B * x_B3[k] + alpha_C * (1 - x_A3[k] - x_B3[k])) - x_B1[k]) + k_1 * exp(-E_1 / (R * x.T_1[k+1])) * x_A1[k] - k_2 * exp(-E_2 / (R * x.T_1[k+1])) * x_B1[k]) * dt == x_B1[k+1]

            dx_A2dt[k=0:N-1], x_A2[k] + ((u.F_1[k+1] / x.V_2[k+1]) * (x_A1[k] - x_A2[k]) + (F_f2[k] / x.V_2[k+1]) * (x_Af - x_A2[k]) - k_1 * exp(-E_1 / (R * x.T_2[k+1])) * x_A2[k]) * dt == x_A2[k+1]
            dx_B2dt[k=0:N-1], x_B2[k] + ((u.F_1[k+1] / x.V_2[k+1]) * (x_B1[k] - x_B2[k]) + (F_f2[k] / x.V_2[k+1]) * (x_Bf - x_B2[k]) + k_1 * exp(-E_1 / (R * x.T_2[k+1])) * x_A2[k] - k_2 * exp(-E_2 / (R * x.T_2[k+1])) * x_B2[k]) * dt == x_B2[k+1]

            dx_A3dt[k=0:N-1], x_A3[k] + ((u.F_2[k+1] / x.V_3[k+1]) * (x_A2[k] - x_A3[k]) - ((F_R[k] + F_R[k] * epsilon) / x.V_3[k+1]) * ((alpha_A * x_A3[k]) / (alpha_A * x_A3[k] + alpha_B * x_B3[k] + alpha_C * (1 - x_A3[k] - x_B3[k])) - x_A3[k])) * dt == x_A3[k+1]
            dx_B3dt[k=0:N-1], x_B3[k] + ((u.F_2[k+1] / x.V_3[k+1]) * (x_B2[k] - x_B3[k]) - ((F_R[k] + F_R[k] * epsilon) / x.V_3[k+1]) * ((alpha_B * x_B3[k]) / (alpha_A * x_A3[k] + alpha_B * x_B3[k] + alpha_C * (1 - x_A3[k] - x_B3[k])) - x_B3[k])) * dt == x_B3[k+1]
        ...
        u_opt = Vector(JuMP.value.(F_f1)), Vector(JuMP.value.(F_f2)), Vector(JuMP.value.(F_R))
        x_pred = Vector(JuMP.value.(x_A1)), Vector(JuMP.value.(x_B1)), Vector(JuMP.value.(x_A2)), Vector(JuMP.value.(x_B2)), Vector(JuMP.value.(x_A3)), Vector(JuMP.value.(x_B3))
    ...
    function subsystem2()
        ...
        @NLconstraints MPC begin
            ...
            dV_1dt[k=0:N-1], V_1[k] + (u.F_f1[k+1] + u.F_R[k+1] - F_1[k]) * dt == V_1[k+1]
            dV_2dt[k=0:N-1], V_2[k] + (u.F_f2[k+1] + F_1[k] - F_2[k]) * dt == V_2[k+1]
            dV_3dt[k=0:N-1], V_3[k] + (F_2[k] - u.F_R[k+1] * epsilon - u.F_R[k+1] - F_3[k]) * dt == V_3[k+1]

            # # Volume hold-up constraints
            holdUp1[k in 0:N], u.F_f1[k+1] + u.F_R[k+1] - F_1[k] == -(V_1[k] - V_1_sp) / tau
            holdUp2[k in 0:N], u.F_f2[k+1] + F_1[k] - F_2[k] == -(V_2[k] - V_2_sp) / tau
            holdUp3[k in 0:N], F_2[k] - F_3[k] - u.F_R[k+1] * epsilon - u.F_R[k+1] == -(V_3[k] - V_3_sp) / tau
        ...
        # Obtain optimal inputs and corresponding obtained state predictions
        u_opt = Vector(JuMP.value.(F_1)), Vector(JuMP.value.(F_2)), Vector(JuMP.value.(F_3))
        x_pred = Vector(JuMP.value.(V_1)), Vector(JuMP.value.(V_2)), Vector(JuMP.value.(V_3))
        return u_opt, x_pred, obj_val
    end
    ...
    function subsystem3()
        ...
        @NLconstraints MPC begin
            ...
            dT_1dt[k=0:N-1], T_1[k] + ((u.F_f1[k+1] / x.V_1[k+1]) * (T_f - T_1[k]) + (u.F_R[k+1] / x.V_1[k+1]) * (T_3[k] - T_1[k]) + Q_1[k] / (rho * cp * x.V_1[k+1]) - (m / cp) * H_1 * k_1 * exp(-E_1 / (R * T_1[k])) * x.x_A1[k+1] - (m / cp) * H_2 * k_2 * exp(-E_2 / (R * T_1[k])) * x.x_B1[k+1]) * dt == T_1[k+1]
            dT_2dt[k=0:N-1], T_2[k] + ((u.F_1[k+1] / x.V_2[k+1]) * (T_1[k] - T_2[k]) + (u.F_f2[k+1] / x.V_2[k+1]) * (T_f - T_2[k]) + Q_2[k] / (rho * cp * x.V_2[k+1]) - (m / cp) * H_1 * k_1 * exp(-E_1 / (R * T_2[k])) * x.x_A2[k+1] - (m / cp) * H_2 * k_2 * exp(-E_2 / (R * T_2[k])) * x.x_B2[k+1]) * dt == T_2[k+1]
        ...
        # Obtain optimal inputs and corresponding obtained state predictions
        u_opt = Vector(JuMP.value.(Q_1)), Vector(JuMP.value.(Q_2)), Vector(JuMP.value.(Q_3))
        x_pred = Vector(JuMP.value.(T_1)), Vector(JuMP.value.(T_2)), Vector(JuMP.value.(T_3))
        return u_opt, x_pred, obj_val
    end
