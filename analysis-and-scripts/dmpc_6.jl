...
function dmpc6_mpc(x_init, x_sp, u_sp, max_coordinations)
    ...
    function subsystem2()
        ...
        @NLconstraints MPC begin
            ...
            # Discretized governing differential equations for temperature
            dT_2dt[k=0:N-1], T_2[k] + ((u.F_1[k+1] / x.V_2[k+1]) * (x.T_1[k+1] - T_2[k]) + (F_f2[k] / x.V_2[k+1]) * (T_f - T_2[k]) + Q_2[k] / (rho * cp * x.V_2[k+1]) - (m / cp) * H_1 * k_1 * exp(-E_1 / (R * T_2[k])) * x_A2[k] - (m / cp) * H_2 * k_2 * exp(-E_2 / (R * T_2[k])) * x_B2[k]) * dt == T_2[k+1]

            # Discretized governing differential equations for concentration A
            dx_A2dt[k=0:N-1], x_A2[k] + ((u.F_1[k+1] / x.V_2[k+1]) * (x.x_A1[k+1] - x_A2[k]) + (F_f2[k] / x.V_2[k+1]) * (x_Af - x_A2[k]) - k_1 * exp(-E_1 / (R * T_2[k])) * x_A2[k]) * dt == x_A2[k+1]
            dx_B2dt[k=0:N-1], x_B2[k] + ((u.F_1[k+1] / x.V_2[k+1]) * (x.x_B1[k+1] - x_B2[k]) + (F_f2[k] / x.V_2[k+1]) * (x_Bf - x_B2[k]) + k_1 * exp(-E_1 / (R * T_2[k])) * x_A2[k] - k_2 * exp(-E_2 / (R * T_2[k])) * x_B2[k]) * dt == x_B2[k+1]
        ...
        # Obtain optimal inputs and corresponding obtained state predictions
        u_opt = Vector(JuMP.value.(F_f2)), Vector(JuMP.value.(Q_2))
        x_pred = Vector(JuMP.value.(T_2)), Vector(JuMP.value.(x_A2)), Vector(JuMP.value.(x_B2))
        return u_opt, x_pred, obj_val
    end
    ...
    function subsystem4()
        ...
        @NLconstraints MPC begin
            ...
            # # Discretized governing differential equations for temperature
            # dT_2dt[k=0:N-1], T_2[k] + ((F_1[k] / V_2[k]) * (x.T_1[k+1] - T_2[k]) + (u.F_f2[k+1] / V_2[k]) * (T_f - T_2[k]) + Q_2[k] / (rho * cp * V_2[k]) - (m / cp) * H_1 * k_1 * exp(-E_1 / (R * T_2[k])) * x_A2[k] - (m / cp) * H_2 * k_2 * exp(-E_2 / (R * T_2[k])) * x_B2[k]) * dt == T_2[k+1]

            # # Discretized governing differential equations for concentration A
            # dx_A2dt[k=0:N-1], x_A2[k] + ((F_1[k] / V_2[k]) * (x.x_A1[k+1] - x_A2[k]) + (u.F_f2[k+1] / V_2[k]) * (x_Af - x_A2[k]) - k_1 * exp(-E_1 / (R * T_2[k])) * x_A2[k]) * dt == x_A2[k+1]
            # dx_B2dt[k=0:N-1], x_B2[k] + ((F_1[k] / V_2[k]) * (x.x_B1[k+1] - x_B2[k]) + (u.F_f2[k+1] / V_2[k]) * (x_Bf - x_B2[k]) + k_1 * exp(-E_1 / (R * T_2[k])) * x_A2[k] - k_2 * exp(-E_2 / (R * T_2[k])) * x_B2[k]) * dt == x_B2[k+1]
        ...
        # Obtain optimal inputs and corresponding obtained state predictions
        u_opt = Vector(JuMP.value.(F_1)), Vector(JuMP.value.(F_2))
        x_pred = Vector(JuMP.value.(V_2))
        return u_opt, x_pred, obj_val
    end
